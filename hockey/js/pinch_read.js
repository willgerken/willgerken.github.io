...
window.IceQ.PinchRead = (function () {
  ...
  const PLAYS = [
    {
      key: 'covered-flat',
      answer: 'pinch',
      cue: 'Rim coming up your wall. Their winger is standing still on the boards. Look high: F3 is above the circles, covering for you. PINCH or PEEL?',
      f3: { x: 13, y: 16 },
    ...
    },
    {
      key: 'nobody-high',
      answer: 'peel',
      cue: 'Same rim, same winger. But look high: F3 is below the dots with everybody else. Nobody is covering the point. PINCH or PEEL?',
      f3: { x: 18, y: 50 },
    ...
    },
    {
      key: 'winger-flying',
      answer: 'peel',
      cue: 'F3 is high this time. But watch their winger: he is already FLYING up the wall with speed, timed to the rim. PINCH or PEEL?',
      f3: { x: 13, y: 16 },
      winger: [{ t: 0, x: 38, y: 46 }, { t: 0.55, x: 39, y: 30 }, { t: 0.9, x: 40, y: 12 }, { t: 1, x: 40, y: 6 }],
      whyPinch: 'He had speed and you were flat. You lose that race every time, and then he is around you. Peel, gap up, make it a 2-on-2.',
      whyPeel: 'Right. He had speed and you did not; a pinch there is a race you lose. You backed off and kept your gap.',
    },
    {
      key: 'bobble',
      answer: 'pinch',
      cue: 'F3 is high. The rim comes to the winger and he BOBBLES it, it is in his feet. That is a loose puck on your wall. PINCH or PEEL?',
      f3: { x: 13, y: 16 },
      winger: [{ t: 0, x: 38, y: 27 }, { t: 1, x: 38, y: 27 }],
      bobbleAt: 0.72,
  ...
  function init(rinkContainer) {
    ...
    function drawScene() {
      ...
      nodes.ow = mk({ color: 'opponent', stickSide: 'L' }); IceQ.Player.face(nodes.ow, 'y-');                                   // their winger on the wall
    ...
    // ---- movement helpers ------------------------------------------------
    const SKATE = 20, RIMS = 46, PASS = 44, SHOT = 62;
    ...
    function playConsequence(res, sig) {
      ...
      return (async function () {
        ...
        if (call === 'pinch') {
          ...
          const wFt = nodeFt(ow);
          const meet = { x: RIM_X - 2.5, y: Math.max(14, Math.min(30, wFt.y - 4)) };
          if (res.correct) {
          ...
          const burst = move(ow, [nodeFt(ow), { x: RIM_X - 1, y: 2 }, { x: 36, y: -8 }], SKATE + 6, sig, true);
          await move(oc, [nodeFt(oc), { x: 6, y: 4 }, { x: 2, y: -8 }], SKATE + 5, sig);
          await burst;
          if (sig.skipped) return;
          await IceQ.Path.animateGoalConsequence(rink, { kind: 'goal', message: p.key === 'nobody-high' ? 'BEAT YOU, NOBODY HIGH. 2-ON-1 THE OTHER WAY.' : 'HE HAD SPEED, YOU DID NOT. 2-ON-1 THE OTHER WAY.', duration: 1.1 });
          return;
        }
        // PEEL: you back off toward the blue line and gap up; the rim goes out.
        const puckNow = puckFt();
        const back = move(you, [YOU_START, { x: 24, y: 1 }], SKATE, sig);
        await slide([puckNow, { x: RIM_X, y: 14 }], RIMS, sig);
        await back;
        if (sig.skipped) return;
        // Winger collects and exits the zone; you are in front of him.
        if (p.key === 'bobble') { /* puck is in his feet; he recovers it */ }
        nodes.puck.position(IceQ.Player.puckPosFor(ow));
    ...
    function showContrastReplay(res, skipSignal) {
      ...
      return (async function () {
        ...
        const cueTxt = p.answer === 'pinch'
          ? (p.key === 'bobble' ? 'LOOSE PUCK + F3 HIGH = PINCH' : 'F3 HIGH + WINGER FLAT = PINCH')
          : (p.key === 'nobody-high' ? 'NOBODY HIGH = PEEL' : 'WINGER FLYING = PEEL');
    ...
    const info = () => ({ rushIdx: playIdx, rush: currentPlay(), totalRushes: PLAYS.length, cue: currentPlay().cue });
    ...
    return {
      ...
      nextRush: nextPlay, currentRushInfo: info, cue: () => currentPlay().cue,
  ...
  function phrasedFeedback(res) {
    ...
    switch (res.kind) {
      case 'froze': return 'You never made a call and the puck rimmed right past you. Pinch or peel, but decide before it gets to you.';
